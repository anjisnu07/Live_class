...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($err)) {
        $sql = "SELECT id, number, password FROM database1 WHERE number = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) == 1) {
                mysqli_stmt_bind_result($stmt, $id, $db_number, $db_password);
...
.closebtn:hover {
  ...
  <strong>Log in first!</strong> You have to login first to join video Discussion.
</div>
...
<input type="text" name="number" class="form-control" id="number" aria-describedby="emailHelp" placeholder="9883185156">
</div>
<div class="form-group">
